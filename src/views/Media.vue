...
const openAddModal = () => {
  form.value = { name: '', category: 'fabric-sample', customerId: '', url: '', notes: '' }
  showModal.value = true
}

const handleSubmit = () => {
  mediaStore.addMedia(form.value)
...
const handleFileUpload = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      form.value.url = e.target.result
      form.value.name = form.value.name || file.name.split('.')[0]
    }
  ...
  <div class="p-4 lg:p-8">
    ...
    <Modal :show="showModal" title="Upload Media" @close="showModal = false">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div v-if="form.url" class="aspect-video bg-gray-100 rounded-lg overflow-hidden">
          <img :src="form.url" class="w-full h-full object-contain" />
        </div>
        <div>
        ...
        <div class="flex gap-3 pt-4">
          ...
          <button type="submit" :disabled="!form.url" class="btn-primary flex-1">Upload</button>
        </div>
      </form>
    </Modal>
